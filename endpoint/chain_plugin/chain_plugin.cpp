...
read_only::get_info2_results read_only::get_info2(const read_only::get_info2_params&) const {
   ...
   result.data.server_version = eosio::utilities::common::itoh(static_cast<uint32_t>(app().version()));
   result.data.chain_id = db.get_chain_id();
   result.data.head_block_num = db.fork_db_head_block_num();
   result.data.last_irreversible_block_num = db.last_irreversible_block_num();
   result.data.last_irreversible_block_id = db.last_irreversible_block_id();
   result.data.head_block_id  = db.fork_db_head_block_id();
   result.data.head_block_time = db.fork_db_head_block_time();
   result.data.head_block_producer = db.fork_db_head_block_producer();
   result.data.virtual_block_cpu_limit = rm.get_virtual_block_cpu_limit();
   result.data.virtual_block_net_limit = rm.get_virtual_block_net_limit();
   result.data.block_cpu_limit = rm.get_block_cpu_limit();
   result.data.block_net_limit = rm.get_block_net_limit();
...
void read_write::push_transaction2(const read_write::push_transaction2_params& params, next_function<read_write::push_transaction2_results> next) {
...
read_only::abi_json_to_bin2_result read_only::abi_json_to_bin2( const read_only::abi_json_to_bin2_params& params )const try {
   ...
   if( abi_serializer::to_abi(code_account->abi, abi) ) {
      ...
      EOS_ASSERT(!action_type.empty(), action_validate_exception, "Unknown action ${action} in contract ${contract}", ("action", params.action)("contract", params.code));
      try {
         result.data.binargs = abis.variant_to_binary(action_type, params.args, abi_serializer_max_time);
      } EOS_RETHROW_EXCEPTIONS(chain::invalid_action_args_exception,
                                "'${args}' is invalid args for action '${action}' code '${code}'. expected '${proto}'",
                                ("args", params.args)("action", params.action)("code", params.code)("proto", action_abi_to_variant(abi, action_type)))
   } else {
...
read_only::get_required_keys2_result read_only::get_required_keys2( const get_required_keys2_params& params )const {
   ...
   elog("transcation:${pretty_input}",("pretty_input",pretty_input));
